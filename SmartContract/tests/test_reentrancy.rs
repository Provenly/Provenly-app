...
//!    Provenly contract. The outer claim must fail (cleanly, with full
...
/// `claim_reward` on the Provenly contract. It implements just enough of
...
impl EvilToken {
    ...
    pub fn transfer(env: Env, _from: Address, _to: Address, _amount: i128) {
        // Re-enter the Provenly contract while the outer claim_reward is
        ...
        let client = ProvenlyContractClient::new(&env, &target);
        client.claim_reward(&quest_id, &submitter, &100);
    }
}

fn setup_contract(env: &Env) -> (Address, ProvenlyContractClient<'_>) {
    let id = env.register_contract(None, ProvenlyContract);
    let client = ProvenlyContractClient::new(env, &id);
    (id, client)
}
