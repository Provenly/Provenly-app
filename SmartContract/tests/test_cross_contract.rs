...
//!
//! This module tests the Provenly contract's ability to interact with other contracts,
...
//! - External contract calling Provenly functions
...
use earn_quest::{
    ...
    ProvenlyContract, ProvenlyContractClient,
};
...
/// Mock External Contract that calls Provenly
#[contract]
pub struct MockExternalContract;

#[contractimpl]
impl MockExternalContract {
    /// External contract registers a quest on Provenly
    pub fn create_quest_on_earn_quest(
        ...
        let earn_quest_client = ProvenlyContractClient::new(&env, &earn_quest_address);
...
impl MockAggregatorContract {
    /// Aggregate user stats from multiple Provenly instances
    pub fn get_total_user_xp(env: Env, earn_quest_addresses: Vec<Address>, user: Address) -> u64 {
        ...
        for earn_quest_addr in earn_quest_addresses.iter() {
            let client = ProvenlyContractClient::new(&env, &earn_quest_addr);
...
fn setup_earn_quest(env: &Env) -> (Address, ProvenlyContractClient<'_>) {
    let contract_id = env.register_contract(None, ProvenlyContract);
...
//================================================================================
// External Contract Calling Provenly Tests
//================================================================================
...
fn test_aggregator_combines_user_xp() {
    ...
    // Setup multiple Provenly instances
